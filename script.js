...
function spawnPowerUps() {
  ...
  const safeLanes = lanes.filter(l => l.type === 'safe' && l.y !== 0);
  ...
  powerUps.forEach(p => {
    if (!p.collected && player.x < p.x + p.width && player.x + player.width > p.x && player.y < p.y + p.height && player.y + player.height > p.y) {
      ...
      if (p.type === 'shield') {
        ...
        doubleScoreTimeout = setTimeout(() => doubleScore = false, 10000);
      }
    }
  });
}
...
function flashLevel(level) {
  ...
  setTimeout(() => levelFlash.style.display = 'none', 1500);
}
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowDown') player.y = Math.min(gameHeight - player.height, player.y + player.speed);
  if (e.key === 'ArrowLeft') player.x = Math.max(0, player.x - player.speed);
  if (e.key === 'ArrowRight') player.x = Math.min(gameWidth - player.width, player.x + player.speed);
  if (player.y < oldY) score += doubleScore ? 20 : 10;
  updateGameInfo();
  if (player.y === 0) {
